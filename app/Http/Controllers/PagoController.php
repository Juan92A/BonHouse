...
class PagoController extends Controller
{
    public function pagar(Request $request)
    {
        ...
        if ($request->has('pago')) {
            ...
            if ($opcion_pago == 'efectivo') {
                ...
                return view('Pago.efectivo', compact('carrito','tipoventa'));
            }
        }
    ...
    public function pagarEvento(Request $request)
    {
        $tipoventa = 1;
        // Verificar la opción de pago seleccionada
        if ($request->has('pago')) {
            ...
            // Si la opción de pago es tarjeta, redirigir a la vista para ingresar los datos de la tarjeta
            if ($opcion_pago == 'tarjeta') {
                $carritoEvento = session('carritoEvento', []);
